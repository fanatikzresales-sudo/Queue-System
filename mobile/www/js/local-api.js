...
(function (global) {
  ...
  const handlers = {
    ...
    '/api/optimize': (_params, body) => {
      ...
      const targetFinalDelayMs = parseOptionalInt(data.final_delay_ms);
      ...
      try {
        let target;
        if (demo) {
          target = sched.create_demo_target({ minutes_from_now: 5.0, tz_key: tzKey });
        } else if (live || !customDate) {
          target = sched.next_walmart_queue_time({ tz_key: tzKey });
        } else {
          const tz = sched.get_timezone(tzKey);
          const dateParsed = DT().fromISO(customDate, { zone: tz });
          if (!dateParsed.isValid) return { status: 400, body: { error: 'Invalid date. Use YYYY-MM-DD format.' } };
          ...
          if (queueTimeOverride) {
            try {
              ...
              return { status: 400, body: { error: 'Invalid queue time.' } };
            }
          } else {
          ...
          target = dateParsed.set({ hour: qHour, minute: qMin, second: qSecond, millisecond: 0 });
        }

        const start = resolveStart(startTime, target, tzKey, demo);
        ...
        const mode = demo ? 'demo' : (customDate && !live ? 'custom' : 'live');
        return {
          status: 200,
          body: { mode, timing_mode: timingMode, ...sched.schedule_to_dict(schedule) },
        };
      } catch (e) {
        return { status: 400, body: { error: e.message } };
      }
