...
(function (global) {
  ...
  const LAST_MIN_SWITCH_CANDIDATES = [5, 4, 3, 2];
  const LAST_MIN_FINAL_DELAYS = [3_000, 2_000, 1_500, 1_000, 800];
  ...
  function parse_drop_plan_mode(value) {
    if (!value) {
      return DropPlanMode.LONG_DROP;
    }
    const normalized = String(value).trim().toLowerCase().replace(/-/g, '_');
    for (const mode of Object.values(DropPlanMode)) {
      if (mode === normalized) {
        return mode;
      }
    }
    throw new Error(`Unknown drop_mode '${value}'. Choose: last_min, long_drop`);
  }
  ...
  function build_schedule(options = {}) {
    const {
      ...
      drop_mode: dropModeIn = DropPlanMode.LONG_DROP,
    } = options;
    ...
    let dropMode = dropModeIn;
    if (typeof dropMode === 'string') {
      dropMode = parse_drop_plan_mode(dropMode);
    }
    ...
    if (targetFinalDelayMs !== null && targetFinalDelayMs !== undefined) {
      ...
      if (switchMinutesBefore !== null && switchMinutesBefore !== undefined) {
        switchCandidates = [switchMinutesBefore];
      } else if (dropMode === DropPlanMode.LAST_MIN) {
        switchCandidates = LAST_MIN_SWITCH_CANDIDATES;
      } else {
      ...
      if (steps === null) {
        throw new Error(
          `Could not build a schedule with final delay ${targetFinalDelayMs} ms ` +
            'for this start time and starting delay. Try a different start time, starting delay, or drop plan mode.',
        );
      }
    } else if (dropMode === DropPlanMode.LAST_MIN) {
      for (const finalMs of LAST_MIN_FINAL_DELAYS) {
        steps = _find_two_step_with_final_delay({
          target: targetDt,
          start: startDt,
          initial_delay_ms: delay,
  ...
  function _append_preset_plan(plans, options) {
    ...
    plans.push({
      ...
      switch_minutes_before: switchMinutesBefore,
      preset_category: presetCategory,
      drop_mode: options.drop_mode || DropPlanMode.LONG_DROP,
    });
  }

  function _preset_plans_for_definitions(options) {
    const {
      target,
      tz_key: tzKey,
      timing_mode: timingModeIn,
      preset_definitions: presetDefinitions,
      switch_candidates: switchCandidates,
      preset_category: presetCategory,
      drop_mode: dropMode,
    } = options;
    ...
    for (const [finalDelayMs, label, description, preferredWindows] of presetDefinitions) {
      const windowOrder = [
        ...new Set([
          ...preferredWindows,
  ...
  function preset_schedules_late_drop(options = {}) {
    const {
      target,
      tz_key: tzKey = 'CDT',
      timing_mode: timingModeIn = TimingMode.INSTANT,
    ...
    const tz = get_timezone(tzKey);
    const targetDt = _ensureTz(target, tz);
    const plans = _preset_plans_for_definitions({
      target,
      tz_key: tzKey,
      timing_mode: timingMode,
  ...
  function find_compatible_custom_starts(options = {}) {
    const {
      target,
      tz_key: tzKey = 'CDT',
      target_final_delay_ms: targetFinalDelayMs,
      timing_mode: timingModeIn = TimingMode.INSTANT,
      drop_mode: dropModeIn = DropPlanMode.LAST_MIN,
    } = options;
    ...
    const switchCandidates =
      dropMode === DropPlanMode.LAST_MIN
        ? LAST_MIN_SWITCH_CANDIDATES
        : DEFAULT_SWITCH_MINUTES_CANDIDATES;
    const optionsOut = [];
    const seen = new Set();

    for (const [windowMin, windowLabel] of ALL_START_WINDOWS) {
      const start = targetDt.minus({ minutes: windowMin });
      for (const startDelay of PRESET_START_DELAY_PREFERENCES) {
        if (startDelay >= windowMin * 60 * 1000) {
          continue;
        ...
        const s1 = steps[0];
        const s2 = steps[1];
        if (s2.delay_ms !== targetFinalDelayMs) {
          continue;
        }

        const commandAt = drop_command_at(s2.at, s1.delay_ms, timingMode);
        if (commandAt < s1.at) {
          continue;
        }

        const startLocal = s1.at.setZone(tz);
        const key = `${startLocal.hour}:${startLocal.minute}:${s1.delay_ms}`;
        if (seen.has(key)) {
          continue;
        }
        seen.add(key);

        const dropMinutesBefore = targetDt.diff(commandAt, 'minutes').minutes;
        optionsOut.push({
          start_time_display: _fmtClock(s1.at, tz),
          start_h: startLocal.hour,
          start_m: startLocal.minute,
    ...
    optionsOut.sort((a, b) => b.minutes_early - a.minutes_early || b.start_delay_ms - a.start_delay_ms);
    return optionsOut;
  }
